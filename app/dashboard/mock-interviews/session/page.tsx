...
import {
  Clock, Send, Mic, MicOff, ChevronRight,
...
const TYPE_CONFIG: Record<string, { color: string; icon: typeof Brain; label: string }> = {
...
export default function InterviewSessionPage() {
  ...
  const [phase, setPhase] = useState<'loading' | 'answering' | 'submitting'>('loading');
  ...
  function toggleVoice() {
    ...
    const SpeechRecognitionAPI = (window as Window & { webkitSpeechRecognition?: typeof SpeechRecognition }).webkitSpeechRecognition || window.SpeechRecognition;
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-1.5">
              {Array.from({ length: MAX_QUESTIONS }).map((_, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full transition-all ${i < questionNumber - 1 ? 'bg-emerald-500' : i === questionNumber - 1 ? 'bg-slate-900 w-4' : 'bg-slate-200'}`}
                />
              ))}
            </div>
          </div>
          ...
          <>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-8 space-y-4">
              <div className="flex items-center gap-3">
                ...
                {questionNumber === 1 && (
                  <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold">Opening</span>
                )}
                {questionNumber > 1 && (
                  <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold">Follow-up</span>
                )}
              </div>
              ...
              {showSTAR && (
                <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 space-y-1">
                  {STAR_TIPS.map((tip, i) => (
                    <p key={i} className="text-sm text-violet-800">{tip}</p>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden">
              ...
              <div className="px-6 pb-2">
                <div className="w-full bg-slate-100 rounded-full h-1">
                  <div
                    className={`h-1 rounded-full transition-all ${wordCount >= 150 ? 'bg-emerald-500' : wordCount >= 50 ? 'bg-amber-500' : 'bg-slate-300'}`}
              ...
              <div className="p-4 border-t border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  ...
                  Answer is saved automatically when you submit
                </div>
                ...
                <div className="flex gap-3">
                  {questionNumber < MAX_QUESTIONS ? (
                    ...
                    <button
                      onClick={() => { submitAnswer({ sessionId: sessionId!, question: currentQuestion!.question, questionType: currentQuestion!.type, answer: answer.trim() }).then(() => handleFinish()); }}
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              {['Be specific with examples', 'Quantify your results', 'Stay concise & structured'].map((tip) => (
                <div key={tip} className="bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-500 font-medium">
                  {tip}
                </div>
              ))}
            </div>
